        ...
        <div class="col-xl-8 col-lg-7">
            <div class="card shadow mb-4">
                <div class="card-header pt-3 pb-1">
                    <h6 class="m-0 font-weight-bold text-primary" style="font-family: doridregular, Arial !important;font-size: 18px;"><strong>تفاصيل علامة</strong></h6>
                </div>
                <hr>
                <div class="card-body">
                    ...
                    <label for="name" class="col-md-2 text-right col-form-label">الطالب<i class="text-danger"> * </i>:</label>
                            <div class="col-mb-3">
                                <div class="">
                                    ...
                                    <select name="student_id" disabled class="form-control select2" id="student_id" tabindex="1">
                                        @foreach ($students as $student)
                                        <!-- <option value="{{ $student->student_id }}">{{ $student->name }}</option> -->
                                        <option readonly <?php if ($student->student_id == $mark->student_id) { ?> selected <?php } ?> value="{{ $student->student_id }}">{{ $student->name }}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-mb-3">
                                <div class="">
                                    <!-- <input type="text" class="form-control valid_number mb-2" name="category" id="category" placeholder="المستودع" value="" tabindex="4"> -->
                                    <select name="course_id" disabled class="form-control select2" id="course_id" tabindex="1">
                                        @foreach ($courses as $course)
                                        <!-- <option value="{{ $course->course_id }}">{{ $course->name }}</option> -->
                                        <option readonly <?php if ($course->course_id == $mark->course_id) { ?> selected <?php } ?> value="{{ $course->course_id }}">{{ $course->name }}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input readonly type="text" type="type" id="mark" name="mark" class="form-control" value="{{ $mark->mark }}">
                                </div>

                        <div class="row">
                            <div class="col-md-2" >
                                <form action="{{route('mark.destroy',$mark->mark_id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                        ...
                                        <div class="modal-dialog modal-confirm">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                  <p>هل تريد حذف العلامة المحدد بشكل نهائي.</p>
                                                 </div>
